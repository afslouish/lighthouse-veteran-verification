...
public class WebExceptionHandler {
  ...
  public ResponseEntity<EmisInaccessibleWsdlErrorApiError> handleEmisInaccessibleWsdlException() {
    return new ResponseEntity<>(
        new EmisInaccessibleWsdlErrorApiError(), createErrorHeaders(), HttpStatus.BAD_GATEWAY);
  }

  /** Return error for inaccessible wsdl exception. */
  @ExceptionHandler({InaccessibleWSDLException.class})
  public ResponseEntity<InaccessibleWsdlErrorApiError> handleInaccessibleWsdlException() {
    return new ResponseEntity<>(
        new InaccessibleWsdlErrorApiError(), createErrorHeaders(), HttpStatus.BAD_GATEWAY);
  }

  /** Return error for EMIS inaccessible wsdl exception. */
  @ExceptionHandler({Exceptions.NoServiceHistoryFoundException.class})
  public ResponseEntity<NoServiceHistoryFoundApiError> handleNoServiceHistoryFoundException() {
    return new ResponseEntity<>(
        new NoServiceHistoryFoundApiError(), createErrorHeaders(), HttpStatus.BAD_GATEWAY);
  }

  /** Return error for soap fault exception. */
  @ExceptionHandler({ServerSOAPFaultException.class})
  public ResponseEntity<ApiError.ServerSoapFaultApiError> handleSoapFaultException(
      ServerSOAPFaultException e) {
    return new ResponseEntity<>(
        new ApiError.ServerSoapFaultApiError(),
        createErrorHeaders(),
